...
class BookshelvesToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=bookshelves">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: bookshelvesSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: bookshelvesSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(BookshelvesToolbar);
