import listActions from 'modules/username/list/usernameListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await UsernameService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.username.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await UsernameService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.username.destroyAll.success'),
      );
